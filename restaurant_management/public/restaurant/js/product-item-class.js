class ProductItem {
      ...
      frappe
        .call({
          ...
          args: {
            ...
            pos_profile,
            force_parent,
            in_menu: this.item_tree.in_menu,
          },
        })
        .then((r) => {
          res(r.message);
  ...
  filter_items({ search_term = this.search_term, item_group = this.parent_item_group } = {}) {
    ...
    this.get_items({ search_value: search_term, page_length: 9999, item_group }).then(
      ({ items, serial_no, batch_no, barcode }) => {
        items.forEach((item) => {
          if (`${item.item_code}`.toLowerCase().includes(search_term)) {
            result_arr.push(item);
          } else if (`${item.item_name}`.toLowerCase().includes(search_term)) {
  ...
  add_item_in_order(item, qty) {
    ...
    if (item.is_customizable === 1) {
      ...
      rate = parse_sub_items
        .filter((sub_item) => sub_item.included === 1)
        .reduce((acc, sub_item) => {
