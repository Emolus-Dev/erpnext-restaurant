...
class Reservation extends DeskForm {
  ...
  check_in() {
    ...
    if (
      !(
        (current_time >= reservation_time && current_time <= reservation_end_time) ||
        reservation_status === 'Waitlisted'
      )
    ) {
      if (table && table.length > 0) {
        frappe.confirm('Reservation time is not in range. Do you want to continue?', () => {
  ...
  make_events() {
    this.on('table_description', 'change', (field) => {
      this.reservation_manage &&
        this.reservation_manage.set_button_status(this.get_value('table_description'), this.doc_name);
    });
              ...
              row.append(
                fields.map((field) => {
                  ...
                  if (field.fieldname == 'customer') {
                    value =
                      "<strong><span class='fa fa-user'></span></strong> " +
