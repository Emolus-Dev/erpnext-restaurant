ProcessManage = class ProcessManage {
  ...
  render_group_container(orders = {}) {
            ...
            <div div class="widget links-widget-box hide" data-group="${data.name}" >
                <div class="widget-head">
                    ...
                    <div class="widget-control">
                        ...
                        <span class="ellipsis">
                            <strong data-name="ordered_time" data-value="${
                              data.ordered_time
                                ...
                                <th colspan="2" style="border-top: 0px;">
                                    <span class="ellipsis">
                                        <div style="width: 100%; height: 30px;">
                                            ...
                                            <span data-name="customer" style="position:relative; top:5px">${
                                              data.customer
                                            }</span>
    ...
    const update_order = (data) => {
      ...
      status_label
        .val(`<i class="${psd.icon} pull-left status-label-icon" style="font-size: 22px"></i> ${psd.status_message}`)
        .css([{ prop: 'background-color', value: psd.color }]);
  ...
  time_elapsed() {
    Object.values(this.orders).forEach((order) => {
      ...
      input.html(
        RMHelper.prettyDate(data.ordered_time, true, (time_elapsed) =>
          this.show_alert_time_elapsed(input, time_elapsed)
        )
      );
    });
