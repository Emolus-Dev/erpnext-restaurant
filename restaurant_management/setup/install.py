...
custom_fields = {
    "POS Profile User": dict(
        ...
        restaurant_permissions=dict(
            ...
            options="Restaurant Permission",
            hidden=1,
            insert_after="Restaurant Permission",
        ),
    ),
    "POS Profile": dict(
        ...
        crm_room=dict(
            label="CRM Room", fieldtype="Link", options="Restaurant Object", insert_after="restaurant_settings"
        ),
        column_break_1=dict(fieldtype="Column Break", insert_after="crm_room"),
        crm_table=dict(
            label="CRM Table", fieldtype="Link", read_only=1, options="Restaurant Object", insert_after="column_break_1"
        ),
        restaurant_menu=dict(
            label="Restaurant Menu", fieldtype="Link", options="Restaurant Menu", insert_after="company"
        ),
    ),
    ...
    "Item": dict(
        ...
        is_customizable=dict(
            label="Is Customizable",
            fieldtype="Check",
            insert_after="customizable",
            description="If checked, the item will be customizable",
        ),
        customizable_options=dict(
            label="Customizable Options", fieldtype="Table", options="Item Customizable", insert_after="is_customizable"
        ),
        section_break_27=dict(fieldtype="Section Break", insert_after="customizable_options", collapsible=1),
        item_type=dict(
            label="Item Type", fieldtype="Select", options="Veg\nNon-Veg\nVeg/Non-Veg", insert_after="section_break_27"
        ),
...
def set_default_process_status():
    status = [
        ...
        dict(
            ...
            color="steelblue",
            message="Whiting",
            action_message="Confirm",
            allows_to_edit_item="0",
        ),
        dict(
            action="Processing",
            icon="fa fa-gear",
    ...
    for doc in custom_fields:
        for field_name in custom_fields[doc]:
            ...
            if test_field is None or field_name != "posa_tax_inclusive":
                CF = (
                    frappe.new_doc("Custom Field") if test_field is None else frappe.get_doc("Custom Field", test_field)
                )

                _values = dict(
                    chain.from_iterable(
                        d.items() for d in (custom_fields[doc][field_name], dict(dt=doc, fieldname=field_name))
                    )
                )
