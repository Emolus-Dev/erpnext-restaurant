...
class DeskForm(Document):
    ...
    def validate_fields(self):
        ...
        for df in self.desk_form_fields:
            ...
            if df.fieldname and (
                df.fieldtype not in no_value_fields and not meta.has_field(df.fieldname) and not df.extra_field
            ):
                missing.append(df.fieldname)

        if missing:
            frappe.throw(
                _("Following fields are missing:") + " in DeskForm " + self.title + "<br>" + "<br>".join(missing)
            )
...
def get_form_data(form_name=None, doc_name=None):
    ...
    if frappe.session.user != "Guest" and not doc_name and not desk_form.allow_multiple:
        doc_name = frappe.db.get_value(desk_form.doc_type, {"owner": frappe.session.user}, "name")
