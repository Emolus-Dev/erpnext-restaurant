...
class RestaurantSettings(Document):
    ...
    def settings_data(self):
        ...
        tax_template = frappe.db.get_value("Sales Taxes and Charges Template", {"company": company})
        pos = self.pos_profile_data()

        if pos.get("has_pos"):
            menu = frappe.db.get_value("Restaurant Menu", pos.get("pos").restaurant_menu, "name")
            menu_items = frappe.get_all("Restaurant Menu Item", filters=dict(parent=menu), fields=["item"])
            groups_of_items = frappe.get_all(
                "Item", "item_group", filters=dict(item_code=("in", [item.item for item in menu_items]))
            )
            ...
            items_groups = [item.item_group for item in groups_of_items] + [
                item.parent_item_group for item in parent_item_groups
            ]
        else:
            menu = None
            menu_items = []
        ...
        return dict(
            ...
            allows_to_edit_item=frappe.get_all("Status Order PC", "name", filters=dict(allows_to_edit_item=1)),
            menu=menu,
    ...
    def pos_profile_data(self):
        ...
        return dict(
            ...
            pos=frappe.get_doc("POS Profile", pos_profile_name) if pos_profile_name is not None else None,
        )
    ...
    def restaurant_access(self, type="Room", more_filters=None):
        ...
        if pos_profile_name is not None:
            permission_parent = frappe.db.get_value(
                "POS Profile User",
                filters=dict(parenttype="POS Profile", parent=pos_profile_name, user=frappe.session.user),
                fieldname="name",
            )
            ...
            restaurant_permissions = (
                frappe.db.get_all("Restaurant Permission", "object_name", filters=permissions_filter)
                if permission_parent
                else []
            )

            if type == "Room":
                restaurant_permissions = frappe.get_all(
                    "Restaurant Object",
                    ("room", "name", "type"),
                    filters=dict(name=("in", [item.object_name for item in restaurant_permissions])),
                )
