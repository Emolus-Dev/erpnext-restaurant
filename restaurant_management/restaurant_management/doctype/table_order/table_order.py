...
class TableOrder(Document):
    ...
    def notify_status(self):
        ...
        if self.status == "Sent" and last_status != "Sent":
            frappe.msgprint(_("Order {0} has been sent to kitchen").format(self.name), indicator="green", alert=True)

        if self.status == "Cancelled" and last_status != "Cancelled":
            frappe.msgprint(_("Order {0} has been cancelled").format(self.name), indicator="red", alert=True)
    ...
    def items_count(self):
        return frappe.db.count(
            "Order Entry Item", filters={"parenttype": "Table Order", "parent": self.name, "qty": (">", "0")}
        )

    @property
    def products_not_ordered_count(self):
        return frappe.db.count(
            "Order Entry Item", filters={"parenttype": "Table Order", "parent": self.name, "status": status_attending}
        )
    ...
    def divide_template(self):
        return frappe.render_template(
            ...
            {"model": self, "items": self.items_list(), "table": self.table},
        )
    ...
    def make_invoice(self, mode_of_payment):
        ...
        for mp in mode_of_payment:
            invoice.append("payments", dict(mode_of_payment=mp, amount=mode_of_payment[mp]))
    ...
    def get_invoice(self, entry_items=None, make=False):
        ...
        for t in set(in_invoice_taxes):
            tax = frappe.db.get_value(
                ...
                dict(parenttype=tax_template, account_head=t),
                ["charge_type", "rate", "amount", "included_in_print_rate"],
                as_dict=True,
            )
        ...
        if self.is_delivery == 1 and self.delivery_branch != 1 and self.address:
            ...
            shipping_data = frappe.db.get_value(
                "Delivery Charges", address, ["default_rate", "shipping_account", "cost_center"], as_dict=True
            )
        ...
        invoice.payments = []
        invoice.append("payments", dict(mode_of_payment="cash", amount=invoice.grand_total))
    ...
    def update_item(self, entry, unrestricted=False, synchronize_on_delete=True):
        if entry["qty"] == 0:
            ...
            if frappe.db.count("Order Entry Item", {"identifier": entry["identifier"]}) == 0:
                ...
                frappe.db.sql(
                    """{base_sql} WHERE `identifier`='{identifier}'""".format(
                        base_sql=base_sql, identifier=entry["identifier"]
                    )
                )
    ...
    def data(self, items=None, last_table=None):
        ...
        return dict(order=short_data, items=dict(data=short_data, items=items))

    def get_delivery_address(self, address=None):
        if not address:
            return {"address": "", "charges": 0}

        _address = frappe.get_doc("Address", address)

        charges = (
            0
            if self.delivery_branch == 1
            else frappe.db.get_value("Delivery Charges", _address.posa_delivery_charges, "default_rate")
        )
