erpnext.PointOfSale.RestaurantController = class {
      ...
      fields: [
        {
          fieldtype: 'Link',
          label: __('Company'),
          default: RM.pos_profile.company,
          options: 'Company',
          fieldname: 'company',
          reqd: 1,
          read_only: 1,
  ...
  save_draft_invoice() {
    ...
    this.frm
      ...
      .then(() => {
        frappe.run_serially([() => frappe.dom.freeze(), () => this.make_new_invoice(), () => frappe.dom.unfreeze()]);
      });
  }
  ...
  get_item_from_frm(item_code, batch_no, uom) {
    ...
    return this.frm.doc.items.find(
      (i) => i.item_code === item_code && (!has_batch_no || (has_batch_no && i.batch_no === batch_no)) && i.uom === uom
    );
  }
